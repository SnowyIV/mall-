...
import com.geekaca.employee.mapper.DeptMapper;
...
import com.geekaca.employee.pojo.Job;
...
//testSelectFind();
    ...
    private static void selectAllEDJName() {
        SqlSession sqlSession = openSession();
        EmployeeMapper mapper = sqlSession.getMapper(EmployeeMapper.class);
        //定义三个对象
        EmployeeMsg emp = new EmployeeMsg();
        Dept dept = new Dept();
        Job job = new Job();
        //设置关联关系
        //员工名字
        ...
        emp.setJob(job);
        job.setJname(null);
        List<EmployeeMsg> emps = mapper.selctDynamicsAll(emp);
        emps.forEach(employeeMsg -> {
...
//        });
//    }
    private static void testFindDeptById() {
        SqlSession sqlSession = openSession();
        DeptMapper sqlSessionMapper = sqlSession.getMapper(DeptMapper.class);
        List<Dept> deptList = sqlSessionMapper.selectDeptById(20);
